...
class GameObject {
 ...
 public:
  ...
  operator bool() const {
    return m_Scene && m_Scene->m_Valid && m_Scene->m_Registry.valid(m_ObjectId);
  }
  ...
  GameObject cloneComponents(ComponentGroup<Component...>) {
    ...
    m_Scene->m_GameObjectIdMap.emplace(entt::to_integral(obj.m_ObjectId),
                                       obj.m_ObjectId);
    auto& transformComponent =
        m_Scene->m_Registry.get<TransformComponent>(obj.m_ObjectId);
    HZN_CORE_INFO("{}, {}, {}", transformComponent.m_Translation.x,
                  transformComponent.m_Translation.y,
                  transformComponent.m_Translation.z);
    return obj;
  }
 ...
 private:
  ...
  GameObject(const entt::entity& object, Scene* scene)
      : m_ObjectId(object), m_Scene(scene) {}

  void isValid() const {
    if (!m_Scene || !m_Scene->m_Valid) {
      HZN_CORE_ERROR("Game Object invalidated! Doesn't belong to any scene!");
      throw std::runtime_error(
          "Game Object invalidated! Doesn't belong to any scene!");
    }
  }

  // if both scenes are not nullptr and both pointers point to the same scene.
  bool sameScene(const GameObject& rhs) const {
    return m_Scene && rhs.m_Scene && &*m_Scene == &*rhs.m_Scene;
  }
