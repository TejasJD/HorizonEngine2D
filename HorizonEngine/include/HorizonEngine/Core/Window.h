...
class Window {
 public:
  ...
  //! returns a void pointer which can be casted to concrete window, based on
  //! the active platform.
  virtual void* getPlatformRawWindow() = 0;

  //! Window creation implemented by the source files based on the active
  //! platform
  static std::shared_ptr<Window> create(uint32_t width = 1366,
                                        uint32_t height = 768,
