...
class CameraController {
 public:
  ...
  //! returns a constant reference to the camera bound to the controller (When
  //! the controller is a constant object).
...
//! Orthographic Camera Controller. Usually used in 2D scenarios. Wrapper around
//! an Orthographic Camera. Allows user to control camera.
class OrthographicCameraController : public CameraController {
 public:
  ...
  //! returns a reference to the constant camera object (when the member
  //! function is called on constant camera object.
  ...
  virtual float getTranslationSpeed() const override {
