...
class Framebuffer {
 public:
  ...
  ///	</summary>
  ///	<returns>Returns a const reference to the properties of the
  ///object.</returns>
  ...
  /// <summary>
  /// Unbind the frame buffer. We unbind the frame buffer in every iteration of
  /// the
  ...
  /// <summary>
  /// Re-creates the frame buffer. Invalidates the previous frame buffer and
  /// generates
  ///	a new one.
  ///	</summary>
  ...
  virtual void clearColorAttachment(uint32_t attachmentIndex,
                                    int value) const = 0;
