...
namespace Hzn {
/// <summary>
/// Represents a Vertex Array. Used to save the type of Vertex Buffers and the
/// Element Buffer, that needs to be bound before drawing. Binding the vertex
/// array in every iteration of the application loop reduces the need to bind
/// the Vertex Buffers, Element Buffer and attribute pointers to those vertex
/// buffers individually per iteration of the application loop.
/// </summary>
class VertexArray {
 public:
  ...
  /// <summary>
  /// Add vertex buffers to the Vertex Array. A Vertex Array maintain attribute
  /// pointers, that point to different attributes in different Vertex Buffers.
  /// Adding a Vertex Buffer to a Vertex Array sets the attribute pointers
  /// associated with that Vertex Buffer in the Vertex Array.
  /// </summary>
  ...
  virtual void setElementBuffer(
      const std::shared_ptr<ElementBuffer>& elementBuffer) = 0;
