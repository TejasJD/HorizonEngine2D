...
#define HZN_ADD_INTERNAL_CALL(name) \
  mono_add_internal_call("Hzn.InternalCalls::" #name, name)

namespace Hzn {
static std::unordered_map<MonoType*, std::function<bool(GameObject)>>
    s_EntityHasComponentFuncs;
static std::unordered_map<MonoType*, std::function<void(GameObject)>>
    s_EntityAddComponentFuncs;
static std::unordered_map<MonoType*, std::function<void(GameObject)>>
    s_EntityRemoveComponentFuncs;

static void Greet() { std::cout << "Hello called from C++" << std::endl; }
...
// FIXME: Should actually get the scene that the game object belongs to but...
// based on the way the scene class is written for now, only a single scene is
// in question at any time (scene needs complete rework).
static MonoString* GameObject_GetScene(uint32_t id) {
  auto scene = Hzn::SceneManager::getActiveScene();
  std::string sceneName =
      scene->getName().substr(0, scene->getName().size() - 6);
  return mono_string_new(mono_domain_get(), sceneName.c_str());
}

static bool GameObject_HasComponent(uint32_t id,
                                    MonoReflectionType* componentType) {
  auto scene = Hzn::SceneManager::getActiveScene();
  ...
  HZN_CORE_ASSERT(s_EntityHasComponentFuncs.find(managedType) !=
                      s_EntityHasComponentFuncs.end(),
                  "couldn't find componentType");
  return s_EntityHasComponentFuncs.at(managedType)(obj);
}

static void GameObject_AddComponent(uint32_t id,
                                    MonoReflectionType* componentType) {
  auto scene = Hzn::SceneManager::getActiveScene();
  HZN_CORE_ASSERT(scene != nullptr, "no scene active!");
  GameObject obj = scene->getGameObjectById(id);
  ...
  MonoType* managedType = mono_reflection_type_get_type(componentType);
  HZN_CORE_ASSERT(s_EntityAddComponentFuncs.find(managedType) !=
                      s_EntityAddComponentFuncs.end(),
                  "couldn't find componentType");

  s_EntityAddComponentFuncs.at(managedType)(obj);
}

static void GameObject_RemoveComponent(uint32_t id,
                                       MonoReflectionType* componentType) {
  auto scene = Hzn::SceneManager::getActiveScene();
  HZN_CORE_ASSERT(scene != nullptr, "no scene active!");
  GameObject obj = scene->getGameObjectById(id);
  ...
  MonoType* managedType = mono_reflection_type_get_type(componentType);
  HZN_CORE_ASSERT(s_EntityRemoveComponentFuncs.find(managedType) !=
                      s_EntityRemoveComponentFuncs.end(),
                  "couldn't find componentType");
...
static MonoString* NameComponent_GetName(uint32_t id) {
  ...
  return mono_string_new(mono_domain_get(),
                         obj.getComponent<NameComponent>().m_Name.c_str());
}
...
static void TransformComponent_GetTranslation(uint32_t id,
...
static void TransformComponent_SetTranslation(uint32_t id,
...
static void TransformComponent_GetRotation(uint32_t id,
...
static void RigidBody2DComponent_GetLinearVelocity(uint32_t id,
                                                   glm::vec2* velocity) {
  auto scene = Hzn::SceneManager::getActiveScene();
  GameObject obj = scene->getGameObjectById(id);

  b2Body* body =
      (b2Body*)obj.getComponent<RigidBody2DComponent>().m_RuntimeBody;
  auto b2Velocity = body->GetLinearVelocity();

  *velocity = {b2Velocity.x, b2Velocity.y};
}

static void RigidBody2DComponent_SetLinearVelocity(uint32_t id,
                                                   glm::vec2* velocity) {
  auto scene = Hzn::SceneManager::getActiveScene();
  GameObject obj = scene->getGameObjectById(id);
...
static void RigidBody2DComponent_ApplyLinearImpulse(uint32_t id,
                                                    glm::vec2* impulse,
                                                    glm::vec2* point,
                                                    bool wake) {
  auto scene = Hzn::SceneManager::getActiveScene();
  ...
  body->ApplyLinearImpulse(b2Vec2(impulse->x, impulse->y),
                           b2Vec2(point->x, point->y), wake);
}

static void RigidBody2DComponent_ApplyLinearImpulseToCenter(uint32_t id,
                                                            glm::vec2* impulse,
                                                            bool wake) {
  auto scene = Hzn::SceneManager::getActiveScene();
  HZN_CORE_ASSERT(scene != nullptr, "no scene active!");
  ...
  (
      []() {
        ...
        MonoType* managedType = mono_reflection_type_from_name(
            managedTypename.data(), ScriptEngine::GetCoreAssemblyImage());
        if (!managedType) {
          HZN_CORE_ERROR("Could not find component type {}", managedTypename);
          return;
        }

        s_EntityHasComponentFuncs[managedType] = [](GameObject obj) {
