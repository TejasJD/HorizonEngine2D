...
struct ScriptData {
  ...
  std::unordered_map<std::string, std::shared_ptr<ScriptClass>>
      gameObjectSubClasses;
  std::unordered_map<uint32_t, std::shared_ptr<ScriptObject>>
      gameObjectScriptInstances;
...
ScriptClass::ScriptClass(const std::string& nameSpace, const std::string& name,
                         bool isCore) {
  if (!isCore) {
    m_MonoClass = mono_class_from_name(ScriptEngine::s_Data->appAssemblyImage,
                                       nameSpace.c_str(), name.c_str());
  } else {
    m_MonoClass = mono_class_from_name(ScriptEngine::s_Data->coreAssemblyImage,
                                       nameSpace.c_str(), name.c_str());
  }
}
...
ScriptObject::ScriptObject(const std::shared_ptr<ScriptClass>& scriptClass,
...
void ScriptEngine::LoadScripts() {
  ...
  MonoClass* gameObjectClass =
      mono_class_from_name(s_Data->coreAssemblyImage, "Hzn", "GameObject");
  ...
  for (int32_t i = 0; i < numTypes; i++) {
    ...
    const char* name = mono_metadata_string_heap(s_Data->appAssemblyImage,
    ...
    MonoClass* monoClass =
        mono_class_from_name(s_Data->appAssemblyImage, nameSpace, name);

    if (monoClass == gameObjectClass) continue;

    bool isGameObject =
        mono_class_is_subclass_of(monoClass, gameObjectClass, false);
    if (!isGameObject) continue;

    std::shared_ptr<ScriptClass> scriptClass =
        std::make_shared<ScriptClass>(nameSpace, name);
    s_Data->gameObjectSubClasses[fullName] = scriptClass;
  }
}

std::shared_ptr<ScriptClass> ScriptEngine::GetGameObjectSubClass(
    const std::string& name) {
  if (s_Data->gameObjectSubClasses.find(name) ==
      s_Data->gameObjectSubClasses.end())
    return nullptr;
...
std::unordered_map<std::string, std::shared_ptr<ScriptClass>>
ScriptEngine::GetGameObjectSubClasses() {
  return s_Data->gameObjectSubClasses;
}

std::shared_ptr<ScriptObject> ScriptEngine::GetGameObjectScriptInstance(
    uint32_t id) {
  if (s_Data->gameObjectScriptInstances.find(id) ==
      s_Data->gameObjectScriptInstances.end())
    return nullptr;
...
bool ScriptEngine::GameObjectSubClassExists(const std::string fullName) {
  return s_Data->gameObjectSubClasses.find(fullName) !=
         s_Data->gameObjectSubClasses.end();
}
...
MonoObject* ScriptEngine::GetManagedInstance(uint32_t id) {
  HZN_CORE_ASSERT(s_Data->gameObjectScriptInstances.find(id) !=
                      s_Data->gameObjectScriptInstances.end(),
                  "Cannot find Script Instance attached to the object");
  return s_Data->gameObjectScriptInstances.at(id)->m_Object;
}

void ScriptEngine::OnStop() { s_Data->gameObjectScriptInstances.clear(); }
...
void ScriptEngine::OnUpdateGameObject(const GameObject& obj, TimeStep ts) {
  ...
  if (s_Data->gameObjectScriptInstances.find(id) !=
    ...
    HZN_CORE_ERROR("Couldn't find script component attached to {}", id);
}
...
void ScriptEngine::ReloadAssembly() {
  ...
  if (project) {
    ...
    auto binPath =
        project->getPath().parent_path().string() + "\\bin\\ScriptAppLib.dll";
    auto loadPath = project->getPath().parent_path().string() +
    ...
    if (std::filesystem::exists(binPath)) {
      ...
      if (!std::filesystem::exists(
              std::filesystem::path(loadPath).parent_path())) {
        std::filesystem::create_directory(
            std::filesystem::path(loadPath).parent_path());
      }

      std::filesystem::copy(binPath, loadPath,
...
void ScriptEngine::startReload() { s_Data->reloadPending = true; }

bool ScriptEngine::isReloadPending() { return s_Data->reloadPending; }

void ScriptEngine::init() {
  s_Data = new ScriptData();
  initMono();
  s_Data->coreAssemblyPath =
      std::filesystem::current_path().string() + "\\Scripts\\ScriptCoreLib.dll";
  LoadCoreAssembly(s_Data->coreAssemblyPath);
  PrintCoreAssemblyTypes();
  ScriptRegistry::registerFunctions();

  s_Data->gameObjectClass =
      std::make_shared<ScriptClass>("Hzn", "GameObject", true);
  // get mono class from the image.
  /*MonoClass* monoClass = mono_class_from_name(s_Data->coreAssemblyImage,
   * "Hzn", "Main");*/
  ScriptRegistry::registerComponents();

  /*std::shared_ptr<ScriptClass> mainClass =
  std::make_shared<ScriptClass>("Hzn", "Main", true); auto instance =
  mainClass->instantiate();
  ...
  // MonoMethod* PrintMessage = mono_class_get_method_from_name(monoClass,
  // "PrintMessage", 0); mono_runtime_invoke(PrintMessage, instance, nullptr,
  // nullptr);

  //// invoke function with params.
  // MonoMethod* PrintInt = mono_class_get_method_from_name(monoClass,
  // "PrintInt", 1); int value = 7;

  // void* params[1] = { &value };
  // mono_runtime_invoke(PrintInt, instance, params, nullptr);

  // MonoMethod* PrintInts = mono_class_get_method_from_name(monoClass,
  // "PrintInts", 2);
  ...
  // MonoString* stringParam = mono_string_new(s_Data->appDomain, "Hello
  // world!");

  // MonoMethod* PrintCustomMessage = mono_class_get_method_from_name(monoClass,
  // "PrintCustomMessage", 1);
