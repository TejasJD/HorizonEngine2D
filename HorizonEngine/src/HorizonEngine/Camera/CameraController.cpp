...
void OrthographicCameraController::onUpdate(TimeStep ts) {
  ...
  if (m_MousePressed) {
    ...
    glm::vec2 mouseDelta = {lastMousePos.x - currentMousePos.x,
                            currentMousePos.y - lastMousePos.y};
    lastMousePos = currentMousePos;
    m_Position += m_TranslationSpeed * 0.3f * ts *
                  glm::vec3{mouseDelta.x, mouseDelta.y, 0.0f};
  }
...
void OrthographicCameraController::onEvent(Event& e) {
  ...
  dispatcher.Dispatch<MouseScrolledEvent>(
      std::bind(&OrthographicCameraController::onMouseScrolled, this,
                std::placeholders::_1));
  dispatcher.Dispatch<WindowResizeEvent>(
      std::bind(&OrthographicCameraController::onWindowResize, this,
                std::placeholders::_1));
  dispatcher.Dispatch<MouseButtonPressedEvent>(
      std::bind(&OrthographicCameraController::onMouseButtonPressed, this,
                std::placeholders::_1));
  dispatcher.Dispatch<MouseButtonReleasedEvent>(
      std::bind(&OrthographicCameraController::onMouseButtonReleased, this,
                std::placeholders::_1));
}
...
bool OrthographicCameraController::onMouseScrolled(MouseScrolledEvent& e) {
  // HZN_CORE_INFO("(xOffset = {0}, yOffset = {1})", e.GetXOffset(),
  // e.GetYOffset());
...
bool OrthographicCameraController::onMouseButtonPressed(
...
bool OrthographicCameraController::onMouseButtonReleased(
