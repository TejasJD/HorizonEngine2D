...
struct RenderData {
  ...
  std::array<std::shared_ptr<Texture2D>, mxtextureSlots>
      textureSlots;  // TODO: have a UUID that represent assets.
...
void Renderer2D::init() {
  ...
  data.shader = Shader::create(
      {{Hzn::ShaderType::VertexShader, "api_assets/shaders/TextureVertex.glsl"},
       {Hzn::ShaderType::FragmentShader,
        "api_assets/shaders/TextureFragment.glsl"}});
  ...
  // it holds a value (index) which tells GPU, which texture slot to sample the
  // texture from.
  std::array<int32_t, data.mxtextureSlots> samplerArray = {};
  for (int32_t i = 0; i < data.mxtextureSlots; ++i)
    samplerArray[i] = i;  // store the slot value (index) in the sampler array.
  data.shader->bind();
  data.shader->setUniform("u_Textures", samplerArray.data(),
                          data.mxtextureSlots);
  ...
  BufferLayout layout = {{ShaderDataType::Vec3f, "a_Pos"},
...
void Renderer2D::beginScene(const SceneCamera2D& camera,
...
void Renderer2D::submitBatch() {
  // bind the textures we have in our array to the texture units in GPU before
  // we draw.
...
void Renderer2D::drawQuad(const glm::mat4& transform,
  ...
  // if this texture is already one of the bound textures, we just use that
  // texture slot for this quad.
  for (uint32_t i = 0; i < data.textureidx; ++i) {
  ...
  // if this is found to be a new texture, we bind it to the available slot and
...
void Renderer2D::drawSprite(const glm::mat4& transform,
                            const RenderComponent& component, int id) {
...
void Renderer2D::drawQuad(const glm::vec2& position, const glm::vec3& size,
...
void Renderer2D::drawQuad(const glm::vec2& position, float angle,
                          const glm::vec3& size, const glm::vec4 color) {
  drawQuad(glm::vec3(position, 0.0f), angle, size, color);
}

void Renderer2D::drawQuad(const glm::vec3& position, float angle,
                          const glm::vec3& size, const glm::vec4 color) {
  const glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) *
...
void Renderer2D::drawSprite(const glm::vec2& position, const glm::vec3& size,
                            const std::shared_ptr<Sprite2D>& sprite,
                            const glm::vec4& color) {
  drawSprite(glm::vec3(position, 0.0f), size, sprite, color);
}

void Renderer2D::drawSprite(const glm::vec3& position, const glm::vec3& size,
                            const std::shared_ptr<Sprite2D>& sprite,
                            const glm::vec4& color) {
  const glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) *
                              glm::scale(glm::mat4(1.0f), size);
...
void Renderer2D::drawSprite(const glm::vec2& position, float angle,
                            const glm::vec3& size,
                            const std::shared_ptr<Sprite2D>& sprite,
                            const glm::vec4& color) {
...
void Renderer2D::drawSprite(const glm::vec3& position, float angle,
                            const glm::vec3& size,
                            const std::shared_ptr<Sprite2D>& sprite,
                            const glm::vec4& color) {
  const glm::mat4 transform = glm::translate(glm::mat4(1.0f), position) *
