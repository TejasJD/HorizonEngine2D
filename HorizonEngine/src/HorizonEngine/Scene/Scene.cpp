...
Scene::Scene(cereal::JSONInputArchive& inputArchive)
  ...
  loader.entities(inputArchive)
      .component<NameComponent, RelationComponent, TransformComponent,
                 RigidBody2DComponent, BoxCollider2DComponent, RenderComponent,
                 CameraComponent, ScriptComponent>(inputArchive);

  // since all valid objects have name components we create a view on name
  // components
  m_Valid = true;
...
void Scene::serialize(cereal::JSONOutputArchive& outputArchive) {
  entt::snapshot{m_Registry}
      ...
      .component<NameComponent, RelationComponent, TransformComponent,
                 RigidBody2DComponent, BoxCollider2DComponent, RenderComponent,
                 CameraComponent, ScriptComponent>(outputArchive);
}
  ...
  if (m_Valid) {
    ...
    for (auto entity : view) {
      ...
      bodyDef.userData.pointer =
          (uintptr_t)&m_GameObjectIdMap[entt::to_integral(entity)];
...
void Scene::onEditorUpdate(OrthographicCamera& camera, TimeStep ts) {
  if (m_Valid) {
    ...
    for (const auto& entity : sprites) {
      ...
      auto sprite = AssetManager::getSprite(
          renderComponent.spritePath,
          {renderComponent.m_Pos.x, renderComponent.m_Pos.y});
...
void Scene::onUpdate(TimeStep ts) {
  ...
  if (m_Valid) {
      ...
      if (view) {
        for (auto entity : view) {
          ...
          if (obj.getParent()) {
            ...
            objLocalMat = glm::translate(
                objLocalMat,
                glm::vec3(position.x, position.y, transform.m_Translation.z));
            objLocalMat =
                glm::rotate(objLocalMat, glm::radians(body->GetAngle()),
                            glm::vec3(0, 0, 1));
            ...
            glm::vec3 skew = glm::vec3(0.0f);
            glm::vec4 perspective = glm::vec4(0.0f);
            glm::decompose(modelMat, scale, orientation, translation, skew,
                           perspective);
    ...
    if (activeCamera) {
      const auto& sprites =
          m_Registry.view<RenderComponent, TransformComponent>();

      Renderer2D::beginScene(*activeCamera, cameraTransform);
      for (const auto& entity : sprites) {
        auto [renderComponent, transformComponent] =
            sprites.get<RenderComponent, TransformComponent>(entity);
        GameObject obj = getGameObjectById(entt::to_integral(entity));

        Renderer2D::drawSprite(obj.getTransform(), renderComponent,
                               (int)entity);
      }
      Renderer2D::endScene();
    }
...
bool Scene::destroyGameObject(GameObject& obj) {
  if (!m_Valid) {
    HZN_CORE_ERROR("Failed to destroy GameObject that has ID: {0}",
                   entt::to_integral(obj.m_ObjectId));
    return false;
  }
...
GameObject Scene::getGameObjectById(uint32_t id) {
  if (!m_Valid) {
    throw std::runtime_error(
        "trying to get game objects from invalidated scene!");
  }
...
GameObject Scene::getGameObjectByName(const std::string& name) {
  ...
  for (int i = 0; i < allIds.size(); i++) {
    ...
    if (nameComponent.m_Name == name)
      return GameObject{m_GameObjectIdMap.find(allIds.at(i))->second, this};
  }
...
std::vector<GameObject> Scene::getGameObjectsByName(const std::string& name) {
  ...
  for (int i = 0; i < allIds.size(); i++) {
    ...
    if (nameComponent.m_Name == name)
      objects.push_back(
          GameObject{m_GameObjectIdMap.find(allIds.at(i))->second, this});
  }
