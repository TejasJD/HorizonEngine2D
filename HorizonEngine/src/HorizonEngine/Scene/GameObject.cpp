...
void GameObject::setParent(GameObject& obj) {
  ...
  auto& relationComponent =
      m_Scene->m_Registry.get<RelationComponent>(m_ObjectId);
...
void GameObject::addChild(GameObject& obj) {
  ...
  m_Scene->m_Registry.get<RelationComponent>(obj.m_ObjectId).m_Parent =
      m_ObjectId;
  relationComponent.m_ChildCount++;

  glm::mat4 transform = getTransform();
  auto& childTransform =
      m_Scene->m_Registry.get<TransformComponent>(obj.m_ObjectId);
...
void GameObject::removeChild(GameObject& obj) {
  ...
  if (hasChild(obj)) {
    ...
    if (prev != entt::null) {
      ...
      // if we are here, it means that 'obj' is the first child of its parent
      // ('*this'). so we just move the first child of '*this' to the next
      // sibling of 'obj'.
      m_Scene->m_Registry.get<RelationComponent>(parent).m_FirstChild =
          m_Scene->m_Registry.get<RelationComponent>(curr).m_Next;
    }

    // break all the relations of 'obj' with the top and the same level of the
    // hierarchy. except with its own children.
...
glm::mat4 GameObject::getTransform() const {
  if (getParent())
    return getParent().getTransform() *
           m_Scene->m_Registry.get<TransformComponent>(m_ObjectId)
               .getModelMatrix();
...
GameObject GameObject::duplicateAsChild() {
  ...
  auto& clonedRel =
      m_Scene->m_Registry.get<RelationComponent>(clonedObject.m_ObjectId);
  ...
  m_Scene->m_Registry.get<RelationComponent>(prev).m_Next =
      clonedObject.m_ObjectId;
...
/*void GameObject::destroy() {
        ...
        if (std::find(m_Scene->m_ObjectsToDelete.begin(),
m_Scene->m_ObjectsToDelete.end(), id) == m_Scene->m_ObjectsToDelete.end())
