...
Project::Project(const std::string& name, const fs::path& directoryPath) {
  ...
  fs::copy(fs::current_path().string() + "\\icons",
           m_Path.parent_path().string() + "\\icons");

  // set up script directories and files.
  fs::create_directories(m_Path.parent_path().string() +
                         "\\ScriptAppLib\\Source");

  // directory for binaries (the dll is watched by the filewatcher in this
  // library).
  fs::create_directory(m_Path.parent_path().string() + "\\bin");
  // directory from which we load dlls (they are copied from bin to load upon
  // rebuild).
  fs::create_directory(m_Path.parent_path().string() + "\\load_target");

  // sample file for now.
  std::ofstream os(m_Path.parent_path().string() +
                   "\\ScriptAppLib\\Source\\Player.cs");
  os.close();

  // copy lua file from executable to the project directory.
  fs::copy(fs::current_path().string() + "\\premake5.lua",
           m_Path.parent_path().string() + "\\premake5.lua");

  // execute premake with the project's lua file as --file argument.
  std::string premake =
      Hzn::App::getApp().getExecutablePath().string() + "\\premake5 --file=";
  std::string command =
      premake + m_Path.parent_path().string() + "\\premake5.lua" + " vs2022";
...
Project::Project(const std::filesystem::path& projectFilePath)
  ...
  /*ScriptEngine::LoadAppAssembly(m_Path.parent_path().string() +
  "\\bin\\ScriptAppLib.dll"); ScriptEngine::PrintAppAssemblyTypes();*/
}
