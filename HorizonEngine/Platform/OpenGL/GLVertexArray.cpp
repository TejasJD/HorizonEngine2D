...
GLVertexArray::GLVertexArray() { glGenVertexArrays(1, &m_VertexArrayId); }

GLVertexArray::~GLVertexArray() { glDeleteVertexArrays(1, &m_VertexArrayId); }

void GLVertexArray::bind() const { glBindVertexArray(m_VertexArrayId); }
  ...
  for (const auto& element : vertexBuffer->getBufferLayout()) {
    switch (element.m_Type) {
      ...
      case Hzn::ShaderDataType::Vec4f: {
        ...
        glVertexAttribPointer(location, element.getCount(),
      ...
      case Hzn::ShaderDataType::Vec4i: {
        glEnableVertexAttribArray(location);
        glVertexAttribIPointer(location, element.getCount(),
                               ShaderDataTypeToGLenum(element.m_Type),
                               vertexBuffer->getBufferLayout().getStride(),
                               (const void*)(size_t)(element.m_Offset));
        break;
...
void GLVertexArray::setElementBuffer(
