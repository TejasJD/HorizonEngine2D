...
GLWindow::GLWindow(const unsigned int& width, const unsigned int& height,
                   const char* const& title, bool maximized)
...
void GLWindow::init() {
  ...
  m_Window = glfwCreateWindow(m_Data.width, m_Data.height, m_Data.title,
                              nullptr, nullptr);
  m_Context = RenderContext::create(m_Window);
  m_Context->init();

  //! set the glfwWindowUserPointer to hold additional data.
  //! This data holds the dimensions, the title and the callback function as
  //! well.
...
void GLWindow::destroy() { glfwDestroyWindow(m_Window); }
}  // namespace Hzn
