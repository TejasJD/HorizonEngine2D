...
//! provide Shaders in terms of type and filepath. Only Vertex and Fragment
//! Shaders supported for now!.
GLShader::GLShader(
    const std::initializer_list<std::pair<ShaderType, std::string>>&
        typeAndPath) {
  if (typeAndPath.size() != 2) {
    HZN_CORE_ERROR(
        "Inapproriate Shader configuration provided, vertex and fragment "
        "shaders supported only!");
...
GLShader::GLShader(const std::string& vertexSource,
                   const std::string& fragmentSource) {
  compileShaders(vertexSource, fragmentSource);
}

void GLShader::compileShaders(const std::string& vertexSource,
                              const std::string& fragmentSource) {
