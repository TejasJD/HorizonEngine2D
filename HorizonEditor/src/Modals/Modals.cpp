...
bool Modals::getNewProJPopup() {
  ...
  bool result = false;
  if (request_NewProject) ImGui::OpenPopup("New Project");

  if (ImGui::BeginPopupModal("New Project", &request_NewProject,
    ...
    if (!canCreateProject) {
      ImGui::Text(
          "Enter valid name / choose valid base directory / Project with the "
          "same name exists in the same directory!");
    }
    ...
    if (!nameString.empty() && !directoryPath.empty()) {
      if (std::filesystem::exists(directoryPathBuffer) &&
          ...
          !std::filesystem::exists(directoryPath.string() + "\\" +
                                   projectNameBuffer)) {
    ...
    if (ImGui::Button("Create", ImVec2(120, 0))) {
      if (canCreateProject) {
        EditorData::m_Project_Active = Hzn::ProjectManager::create(
            projectNameBuffer, std::filesystem::path(directoryPathBuffer));
...
bool Modals::getNewScenePopup() {
  ...
  bool success = false;
  if (request_NewScene) ImGui::OpenPopup("New Scene");

  if (ImGui::BeginPopupModal("New Scene", &request_NewScene,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
...
void Modals::openProject() {
  ...
  currentScenePath = (EditorData::s_Scene_Active
                          ? EditorData::s_Scene_Active->getFilePath().string()
                          : std::string());

  for (const auto& entry :
       std::filesystem::recursive_directory_iterator(iconPath)) {
    // create texture of directory icon
    if (!entry.is_directory() &&
        entry.path().string().find("DirectoryIcon.png") != std::string::npos) {
      folderIcon = Hzn::Texture2D::create(entry.path().string());
    }

    // create texture of file icon
    if (!entry.is_directory() &&
        entry.path().string().find("FileIcon.png") != std::string::npos) {
