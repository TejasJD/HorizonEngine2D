...
void ContentBrowser::OnImGuiRender() {
  ...
  if (!Modals::projectRootFolder.empty()) {
    if (Modals::m_CurrentDirectory !=
        std::filesystem::path(m_ProjectRootFolder)) {
      if (ImGui::Button("<-")) {
    ...
    std::vector fileFilter{
        ".ini", ".hzn", ".sln", ".csproj", ".lua",
    };

    for (auto& entry :
         std::filesystem::directory_iterator(Modals::m_CurrentDirectory)) {
      if (std::find(fileFilter.begin(), fileFilter.end(),
                    entry.path().extension().string()) != fileFilter.end()) {
        continue;
      }
      if (entry.is_directory() &&
              entry.path().string().find("bin") != std::string::npos ||
          entry.path().string().find("icons") != std::string::npos ||
          entry.path().string().find("load_target") != std::string::npos) {
        continue;
      }
      ...
      if (entry.path().extension().string().find("meta") != std::string::npos) {
        ...
        std::string spriteSheetPath =
            entry.path().filename().replace_extension().string() + ".png";

        if (Hzn::AssetManager::spriteStorage.find(spriteSheetPath) ==
            Hzn::AssetManager::spriteStorage.end()) {
          auto width =
              Hzn::AssetManager::getTexture(spriteSheetPath)->getWidth();
          auto height =
              Hzn::AssetManager::getTexture(spriteSheetPath)->getHeight();
          // (0, 0) to (width, height).
          ...
          for (int i = 0;
               ...
               width /
                   std::stof(
                       Hzn::AssetManager::spriteFormat.find("width")->second);
               ++i) {
            for (int j = 0;
                 j < height / std::stof(Hzn::AssetManager::spriteFormat
                                            .find("height")
                                            ->second);
                 ++j) {
              std::string sheetPathPos = spriteSheetPath + "-;" +
                                         std::to_string(i) + ";" +
                                         std::to_string(j);
              Hzn::AssetManager::spriteStorage[sheetPathPos] =
                  Hzn::Sprite2D::create(
                      Hzn::AssetManager::getTexture(spriteSheetPath), {i, j},
                      {std::stof(Hzn::AssetManager::spriteFormat.find("width")
                                     ->second),
                       std::stof(Hzn::AssetManager::spriteFormat.find("height")
                                     ->second)});
            }
          }
        }
      ...
      if (entry.path().parent_path().string().find("sprites") !=
          std::string::npos) {
        ImGui::ImageButton(filenameString.c_str(),
                           (ImTextureID)(uint64_t)icon->getId(),
      ...
      if (ImGui::BeginDragDropSource()) {
        ...
        ImGui::SetDragDropPayload("CONTENT_BROWSER_ITEM", &filename,
                                  sizeof(filename));
        ImGui::EndDragDropSource();
      }
      if (ImGui::IsItemHovered() &&
          ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left)) {
        if (entry.is_directory()) Modals::m_CurrentDirectory /= path.filename();
      }
      ...
      if (request_Rename && selected_file == path.string()) {
        ...
        for (const auto& entry :
          ...
          if (std::filesystem::exists(
                  (Modals::m_CurrentDirectory.string() + "\\" + newName))) {
            exist = true;
          }
        }
        ...
        if (ImGui::IsItemHovered() &&
          ...
          if (!newName.empty() && path.filename().string() == newName) {
            ...
            std::filesystem::rename(
                selected_file,
                std::filesystem::path(selected_file).parent_path().string() +
                    "\\" + newName);
    ...
    if (!itemWasHovered && ImGui::IsWindowHovered() &&
        ImGui::IsMouseClicked(ImGuiMouseButton_Right)) {
    ...
    if (request_emptyError) ImGui::OpenPopup("Rename error - empty");
    ...
    if (request_existsError) ImGui::OpenPopup("Rename error - exists");
    ...
    if (request_delete) ImGui::OpenPopup("delete file or folder");
  ...
  // new file popup
  if (request_NewFile) ImGui::OpenPopup("NewFile");

  if (ImGui::BeginPopupModal("NewFile", &request_NewFile,
  ...
  // new folder popup
  if (request_NewFolder) ImGui::OpenPopup("NewFolder");

  if (ImGui::BeginPopupModal("NewFolder", &request_NewFolder,
                             ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    for (const auto& entry :
         std::filesystem::directory_iterator(Modals::m_CurrentDirectory)) {
      if (std::filesystem::exists(
              (Modals::m_CurrentDirectory.string() + "\\" + folderName))) {
        exist = true;
      }
    }
    ...
    if (ImGui::Button("Create", ImVec2(120, 0))) {
      // create new project and set it as active project.
      if (!folderName.empty() && !exist) {
        std::filesystem::create_directory(Modals::m_CurrentDirectory.string() +
                                          "\\" + folderName);
        memset(fileNameBuffer, '\0', sizeof(fileNameBuffer));
        request_NewFolder = false;
      }
